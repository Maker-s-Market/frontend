...
export const ProductForm = (props) => {
        ...
        const mockCategories = [
            {name: "Cenas", slug: "coisas"},
            {name: "Cenas1", slug: "coisas1"},
            {name: "Cenas2", slug: "coisas2"},
            {name: "Cenas3", slug: "coisas3"},
            {name: "Cenas4", slug: "coisas4"}
        ]
        ...
        const addCategory = () => setSelectedCategories((prevState) => {
                if (!prevState.some(category => category === categoryOption))
                    return [...prevState, categoryOption]
                return prevState
            }
        )
                    ...
                    <div className={"join"}>
                        <select className="select select-accent max-w-xs join-item"
                            ...
                            {mockCategories.map((item, index) => {
                                return <option key={index} name={item.slug}>{item.name}</option>
                            })}
                        </select>
